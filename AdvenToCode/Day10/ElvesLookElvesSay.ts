...
const lookAndSay = (sequence: string): string => {
  let result = '';
  ...
  for (let i = 0; i < splitSequence.length; i++) {
    let count = 1;
    while (splitSequence[i] === splitSequence[i + 1]) {
      count++;
      i++;
    }
    result += count + splitSequence[i];
  }

  return result;
};

const processSequence = (sequence: string, times: number): number => {
...
// Part 1
// console.log(processSequence('1321131112', 40));
// Part 2
console.log(processSequence('1321131112', 70));
